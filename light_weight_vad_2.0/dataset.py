...
# Functions
##############################################################################
def get_dataloader(phase, sample_rate=DATA_REQUIRED_SR, max_audio_length=None, num_workers=0, batch_size=4, csv_file=None, n_fft=510, hop_length=160, win_length=400):
    print('Mode:', phase)
...
class AudioVisualAVSpeechMultipleVideoDataset(Dataset):
    ...
    def __get_input(self, path, max_audio_length):
        ...
        filter_bank = filter_bank_with_audio(snd, nfilt=30)
...
def test():
    print('In test')
    data_csv_path='/home3/huydd/vad_dataset/vad_data_team_data_gan_17_2_2022/vad_data.csv'
    # dataloader = get_dataloader(PHASE_TRAINING, batch_size=1, num_workers=20, dataset_json=data_json_path)
    dataloader = get_dataloader(PHASE_TESTING, sample_rate=8000, batch_size=10,csv_file=data_csv_path, hop_length=80)
